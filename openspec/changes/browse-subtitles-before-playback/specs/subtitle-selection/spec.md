...
vlcq SHALL render an always-visible, focusable subtitle status in the same line immediately after each supported, canonical Files and Queue video filename beneath the active library root. The inline subtitle status, rather than the video's general context menu, SHALL own mouse and keyboard subtitle interaction. Visible video rows SHALL resolve through bounded background inspection, without scanning off-screen or unrelated library media. A resolved status SHALL display only the full literal embedded subtitle title or sidecar filename as white text on a compact gray background, without circle, star, or other status glyphs; language, embedded/sidecar source, characteristics, active/planned explanation, and alternatives SHALL remain in the picker rather than being repeated in the row. No match or inspection failure SHALL use a short textual state without claiming that an inactive selection is active. Activating or right-clicking the subitem SHALL asynchronously open subtitle inspection without loading, enqueueing, or playing inactive media and SHALL list Off, every embedded subtitle stream reported by the local media inspector, and every safely associated sidecar subtitle. For current media, vlcq SHALL reconcile offline candidates with tracks reported by the same owned VLC playback generation before issuing a playback command. The picker SHALL distinguish embedded and sidecar candidates, present available language, title or sidecar variant, and recognized characteristics without inventing missing metadata. When VLC cannot identify its exact active stream, vlcq SHALL continue to identify VLC's preserved current/default state rather than guessing. Discovery and picker opening SHALL NOT change queue order, playback position, or media files.

#### Scenario: Subtitle status is always visible
- **WHEN** vlcq renders a supported Files or Queue video row
- **THEN** it keeps the video on one line, renders the subtitle's literal name immediately after the filename as a white-on-gray badge without a status glyph, and resolves visible-row MKV metadata in the background
