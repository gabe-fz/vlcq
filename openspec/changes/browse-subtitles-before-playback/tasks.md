...
- [x] 3.5 Keep every Files and Queue video on one line with an always-visible focusable subtitle name immediately after the filename, resolve displayed-row MKV metadata through bounded background inspection, show only the literal embedded title or sidecar filename plus active/selected marker, keep metadata in the picker, remove the duplicate action from general video menus, and verify inline layout, background resolution, duplicate-menu removal, and mouse/keyboard picker tests pass.
