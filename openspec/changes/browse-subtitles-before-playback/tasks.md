...
- [x] 3.5 Keep every Files and Queue video on one line with an always-visible focusable white-on-gray subtitle name immediately after the filename, resolve displayed-row MKV metadata through bounded background inspection, show only the literal embedded title or sidecar filename without status glyphs, keep metadata in the picker, style folders with semantic filename colors on text-width dark mustard badges distinct from row hover, remove the duplicate action from general video menus, and verify inline layout, background resolution, styling, duplicate-menu removal, and mouse/keyboard picker tests pass.
