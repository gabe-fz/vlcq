...
Before implementation, characterize the installed VLC 3 HTTP interface and lock its actual response and selection command into fixtures and an opt-in integration test. The adapter may support explicitly recognized VLC 3 payload variants, but unknown structures fail as unavailable rather than being guessed. The supported macOS VLC 3 status payload enumerates streams without exposing which subtitle stream is selected. Until vlcq successfully applies a generation-local choice, the picker therefore identifies the active state as `VLC current/default · exact track not reported` rather than guessing a track or Off. Payload variants that expose active flags identify the exact choice and are re-read after a command so inconsistent reported state is treated as a failed selection.
